...
struct MessageHeaderFrame
  ...
  MessageHeaderFrame(ProtocolV2 &protocol,
    ...
    fill_preamble({
      ...
      segment_t{ front_bl.length(), 8 },
      segment_t{ middle_bl.length(), 8 },
      segment_t{ data_bl.length() + 16, segment_t::DEFERRED_ALLOCATION },
    }, {});
    ...
    this->payload = \
      protocol.session_stream_handlers.tx->authenticated_encrypt_final();
    ceph_assert(exp_size == this->payload.length());
    //((uint32_t*)this->payload.c_str())[3] = 0xdeadbeef;
  }

  MessageHeaderFrame(ProtocolV2 &protocol, ceph::bufferlist&& cipher_bl)
...
CtPtr ProtocolV2::handle_message() {
  ...
  MessageHeaderFrame header_frame(*this, std::move(rx_segments_data[0]));
