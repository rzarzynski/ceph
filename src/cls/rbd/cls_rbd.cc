...
int object_map_update(cls_method_context_t hctx, bufferlist *in, bufferlist *out)
{
  ...
  if (updated) {
    ...
    CLS_LOG(20, "object_map_update: %llu~%llu -> %llu",
	    ...
	    static_cast<unsigned long long>(object_map.get_data_buffer_offset()
					    + byte_offset));
    bufferlist update;
    update.substr_of(object_map.get_data(), byte_offset, byte_length);
    r = cls_cxx_write(hctx, object_map.get_data_buffer_offset() + byte_offset,
