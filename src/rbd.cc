...
static int export_diff_cb(uint64_t ofs, size_t _len, int exists, void *arg)
{
  ...
  if (exists) {
    // read block
    bl.clear();
    r = ec->image->read2(ofs, len, bl, LIBRADOS_OP_FLAG_FADVISE_DONTNEED);
    if (r < 0)
      return r;
    r = bl.write_fd(ec->fd);
    if (r < 0)
...
public:
      ...
      m_offset(offset)
  {
    ...
    int op_flags = LIBRADOS_OP_FLAG_FADVISE_SEQUENTIAL |
		   LIBRADOS_OP_FLAG_FADVISE_DONTNEED; 
    int r = image.aio_write2(m_offset, bl.length(), bl, m_aio_completion,
			     op_flags);
    if (r < 0) {
...
static int do_import_diff(librbd::Image &image, const char *path)
{
  ...
  while (true) {
	...
	image.write2(off, len, data, LIBRADOS_OP_FLAG_FADVISE_DONTNEED);
      } else {
