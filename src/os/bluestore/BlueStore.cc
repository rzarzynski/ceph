  ...
  float bytes_per_onode)
{
  ...
  if (current <= target_bytes) {
    trace_trim_target(30, bluestore_cache,
     byte_u_t, target_bytes, byte_u_t(target_bytes),
     float, target_meta_ratio, target_meta_ratio,
     float, target_data_ratio, target_meta_ratio,
     byte_u_t, target_meta, byte_u_t(target_meta),
     byte_u_t, target_buffer, byte_u_t(target_buffer),
     byte_u_t, current, byte_u_t(current),
     byte_u_t, current_meta, byte_u_t(current_meta),
     byte_u_t, current_buffer, byte_u_t(current_buffer),
     "shard target %s meta/data ratios %f + %s (%s + %s), current %s (%s + %s)");
    return;
  }
...
//	   << max_buffer << " buffer"
//	   << dendl;
  // TODO: LTTng doesn't allow more than 10 parameters
  trace_trim_shard_target(30, bluestore_cache,
   byte_u_t, target_bytes, byte_u_t(target_bytes),
   float, target_meta_ratio, target_meta_ratio,
   byte_u_t, target_meta, byte_u_t(target_meta),
   byte_u_t, target_buffer, byte_u_t(target_buffer),
   ...
   byte_u_t, current_meta, byte_u_t(current_meta),
   byte_u_t, current_buffer, byte_u_t(current_buffer),
   byte_u_t, need_to_free, byte_u_t(need_to_free),
   byte_u_t, free_meta, byte_u_t(free_meta),
   byte_u_t, free_buffer, byte_u_t(free_buffer),
   "shard target %s ratio %s (%s + %s), current %s (%s + %s), need_to_free (%s + %s) -> MISSING INFO");
...
int BlueStore::_set_cache_sizes()
{
  ...
  if (cache_size > 0 && cache_kv_min >= 0) {
    ...
    if (cache_kv_min_ratio > cache_kv_ratio) {
      trace_disable_cache_kv(1, bluestore,
       double, cache_kv_min_ratio, cache_kv_min_ratio,
       float, cache_kv_ratio, cache_kv_ratio,
       "kv_min_ratio %f > kv_ratio %f""kv_min_ratio %f > kv_ratio %f");
...
int BlueStore::_open_bdev(bool create)
{
  ...
  if (dev_size < cct->_conf->bluestore_bluefs_min) {
    trace_disable_bluefs_min(1, bluestore,
     ...
     "main device size %s is too small, disable bluefs_min for now");
