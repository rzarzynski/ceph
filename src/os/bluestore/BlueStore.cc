...
void BlueStore::collect_metadata(map<string,string> *pm)
{
  ...
  if (bluefs) {
    (*pm)["bluefs"] = "1";
    // this computed value is for backward compatibility only
    bool single_shared_device = \
      !bluefs_layout.dedicated_db && !bluefs_layout.dedicated_wal;
    (*pm)["bluefs_single_shared_device"] = \
      stringify((int)single_shared_device);
...
void BlueStore::_kv_sync_thread()
{
  ...
  while (true) {
	...
	break;
      ...
      if (bluefs && !bluefs_layout.dedicated_db &&
