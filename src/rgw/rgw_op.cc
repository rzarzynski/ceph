                                                ...
                                                map<int, string>& temp_url_keys)
{
 ...
 for(riter = rmattr_names.begin(); riter != rmattr_names.end(); ++riter) {
    ...
    std::cout << "attr to del: " << name << std::endl;
    if (name.compare(RGW_ATTR_TEMPURL_KEY1) == 0) {
...
void RGWPutMetadataAccount::execute()
{
  ...
  rgw_get_request_metadata(s->cct, s->info, attrs);
  rgw_read_user_buckets_metadata(store, s->user.user_id, orig_attrs, &acct_op_tracker);
  ...
  /* Handle the TempURL-related stuff. */
  {
    ...
    if (!temp_url_keys.empty()) {
      ...
      if ((ret = handle_temp_url_update(temp_url_keys)) < 0) {
        return;
      }
    }
  }

  ret = store->set_attrs(NULL, obj, attrs, &rmattrs, &acct_op_tracker);
}
