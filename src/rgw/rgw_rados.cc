...
int RGWRados::Bucket::UpdateIndex::cancel()
{
  ...
  int ret = guard_reshard(&bs, [&](RGWRados::BucketShard *bs) -> int {
...
struct BILogUpdateBatchFIFO {
  static constexpr auto BILOG_FIFO_SUFFIX = \
    RGWSI_BILog_RADOS_CLSFIFO::BILOG_FIFO_SUFFIX;
...
// NOTE: header.ver
//    rc = log_index_operation(op.key, op.op, op.tag, op.meta.mtime, op.ver,
...
#define MAX_ECANCELED_RETRY 100
  int ret = with_bilog<CLSRGWLinkOLH<DeleteMarkerV>>(
                                  ...
                                  uint64_t olh_epoch, optional_yield y, rgw_zone_set *_zones_trace)
{
  ...
  const auto zones_trace = \
    get_zones_trace(svc.zone->get_zone(), bucket_info.bucket, _zones_trace);
  int ret = with_bilog<CLSRGWUnlinkInstance>(
    [&] (auto op_issuer, auto bilog_handler) {
      int i;
      for (i = 0; i < MAX_ECANCELED_RETRY; i++) {
        if (ret == -ECANCELED) {
