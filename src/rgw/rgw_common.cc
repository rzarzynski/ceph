                            ...
                            const int perm)
{
  ...
  return user_acl->verify_permission(s->user.user_id, perm, perm);
}
                              ...
                              const int perm)
{
  ...
  if (bucket_acl->verify_permission(s->user.user_id, perm, perm))
    return true;
                              ...
                              const int perm)
{
  ...
  if (bucket_acl->verify_permission(s->user.user_id, swift_perm, swift_perm))
    return true;

  if (!user_acl)
    return false;

  return user_acl->verify_permission(s->user.user_id, swift_perm, swift_perm);
}
