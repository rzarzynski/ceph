         ...
         RGWUserInfo& old_info, RGWUserInfo& new_info, std::string *err_msg)
{
  ...
  if (!old_info.user_id.empty() &&
    ...
    if (old_info.user_id.default_tenant != new_info.user_id.default_tenant) {
      ldout(store->ctx(), 0) << "ERROR: tenant mismatch: " << old_info.user_id.default_tenant << " != " << new_info.user_id.default_tenant << dendl;
      return false;
    }
                           ...
                           RGWStorageStats *stats = NULL)
{
  ...
  f->dump_string("tenant", info.user_id.default_tenant);
