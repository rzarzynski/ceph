...
// CLS FIFO
void RGWSI_BILog_RADOS_CLSFIFO::init(RGWSI_BucketIndex_RADOS *bi_rados_svc)
{
}

int RGWSI_BILog_RADOS_CLSFIFO::log_trim(const RGWBucketInfo& bucket_info,
                                        int shard_id,
                                        string& marker)
{
  if (shard_id > 0) {
...
int RGWSI_BILog_RADOS_CLSFIFO::log_start(const RGWBucketInfo& bucket_info,
                                         int shard_id)
{
  if (shard_id > 0) {
    // the initial implementation does support single shard only.
...
int RGWSI_BILog_RADOS_CLSFIFO::log_stop(const RGWBucketInfo& bucket_info,
                                        int shard_id)
{
  if (shard_id > 0) {
    // the initial implementation does support single shard only.
...
RGWSI_BILog_RADOS_CLSFIFO::_open_fifo(const RGWBucketInfo& bucket_info)
{
  return {};
}

int RGWSI_BILog_RADOS_CLSFIFO::log_list(const RGWBucketInfo& bucket_info,
                                        int shard_id,
...
int RGWSI_BILog_RADOS_CLSFIFO::log_get_max_marker(
  ...
  RGWSI_BucketIndex_RADOS* const bi_rados_svc)
{
  ...
  backend_clsfifo.init(bi_rados_svc);
}
