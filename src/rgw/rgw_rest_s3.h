...
class RGWPostObj_ObjStore_S3 : public RGWPostObj_ObjStore {
  ...
  const rgw::auth::registry::StrategyRegistry* auth_registry_ptr = nullptr;
...
public:
  ...
  int verify_requester(const rgw::auth::registry::StrategyRegistry& auth_registry) {
...
public:
  RGWHandler_Auth_S3(const rgw::auth::registry::StrategyRegistry& auth_registry)
    : RGWHandler_REST(),
...
public:
  ...
  RGWHandler_REST_S3(const rgw::auth::registry::StrategyRegistry& auth_registry)
    : RGWHandler_REST(),
      auth_registry(auth_registry) {
  }
