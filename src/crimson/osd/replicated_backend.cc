  ...
  eversion_t need)
{
  ...
  return seastar::do_with(PushOp(), get_shards_to_push(soid),
    [this, soid, need, &wfor](auto& pop, auto& shards) {
    return [this, soid, need, &wfor] {
	...
	pulls.push_back(po);
	msg->set_pulls(&pulls);
	(void)shard_services.send_to_osd(pi.from.osd,
					 std::move(msg),
					 pg.get_osdmap_epoch());
    ...
    }().then([this, &pop, &shards, soid, need, &wfor]() mutable {
      return [this, &wfor, soid] {
	if (!wfor.obc) {
	  return pg.get_or_load_head_obc(soid).safe_then(
	    ...
	    if (!existed) {
	      ...
	      wfor.obc->put_lock_type(RWState::RWEXCL);
	    }
	    assert(wfor.obc->get_recovery_read());
	    return seastar::make_ready_future<>();
	  }, crimson::osd::PG::load_obc_ertr::all_same_way(
	      [this, &wfor, soid](const std::error_code& e) {
	      auto [obc, existed] =
	...
	return seastar::now();
      }().then([this, soid, need, &pop, &shards] {
	return prep_push(soid, need, &pop, shards);
      });
    }).handle_exception([this, soid, &wfor](auto e) {
      auto& wfor = recovering[soid];
      if (wfor.obc)
	wfor.obc->drop_recovery_read();
      recovering.erase(soid);
      return seastar::make_exception_future<>(e);
    }).then([this, &pop, &shards, soid] {
      return seastar::parallel_for_each(shards, [this, &pop, soid] (auto j) {
	...
	(void)shard_services.send_to_osd(j->first.osd, std::move(msg), pg.get_osdmap_epoch());
	return recovering[soid].wait_for_pushes(j->first);
      });
    }).then([this, &pop, soid, &wfor] {
  ...
  eversion_t need)
{
  ...
  return seastar::parallel_for_each(pg.get_acting_recovery_backfill(),
    ...
    if (iter->second.is_missing(soid)) {
      ...
      (void)shard_services.send_to_osd(shard.osd, std::move(msg), pg.get_osdmap_epoch());
  ...
  epoch_t epoch_frozen)
{
  ...
  pg.on_local_recover(soid, _recovery_info, is_delete, t);
...
{
  logger().debug("{}: {}, {}", __func__, soid, need);
  return load_metadata(soid).safe_then([this]
    (auto lomt) {
    if (lomt->os.exists) {
      return seastar::do_with(ceph::os::Transaction(),
	[this, lomt = std::move(lomt)](auto& txn) {
	return remove(lomt->os, txn).then([this, &txn]() mutable {
  ...
  const hobject_t &soid, eversion_t need)
{
  ...
  return seastar::do_with(object_stat_sum_t(),
	...
	if (!object_missing) {
      ...
      pg.on_global_recover(soid, stat_diff, true);
      return seastar::make_ready_future<>();
    });
  });
}
  ...
  const std::list<std::map<pg_shard_t, pg_missing_t>::const_iterator>& shards)
{
  ...
  return seastar::do_with(std::map<pg_shard_t, interval_set<uint64_t>>(),
    [this, soid, need, pop, &shards](auto& data_subsets) {
    return seastar::parallel_for_each(shards,
      [this, soid, need, pop, &data_subsets](auto j) mutable {
      auto& wfor = recovering[soid];
      auto& obc = wfor.obc;
	...
	logger().debug("calc_head_subsets {} data_subset {}", soid, data_subset);
      ...
      logger().debug("prep_push: {} to {}", soid, j->first);
	...
	HAVE_FEATURE(pg.min_peer_features(), SERVER_OCTOPUS);
      ...
      .then([this, soid, j](auto new_progress) {
	auto& wfor = recovering[soid];
	auto& pi = wfor.pushing[j->first];
...
seastar::future<ObjectRecoveryProgress> ReplicatedBackend::build_push_op(
  ...
  return seastar::do_with(ObjectRecoveryProgress(progress),
	...
	return seastar::make_ready_future<>();
      ...
      return read(oi, 0, pop->data_included.range_end(),
		  0, 0, 0);
...
seastar::future<> ReplicatedBackend::handle_pull(Ref<MOSDPGPull> m)
{
  logger().debug("{}: {}", __func__, *m);
  vector<PullOp> pulls;
  m->take_pulls(&pulls);
