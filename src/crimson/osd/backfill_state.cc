  ...
  for (const auto& bt : ps().get_backfill_targets()) {
    ...
    if (check == bs().backfill_info.begin && check == pbi.begin) {
      if (pbi.objects.begin()->second != obj_v) {
        bs().progress_tracker->enqueue_push(check);
        ls().enqueue_push(bt, check, obj_v);
      } else {
        // it's fine, keep it!
      }
      result.pbi_targets.insert(bt);
  ...
  const pg_stat_t&)
{
  ...
  if (Enqueuing::all_enqueued(ps(),
    ...
    update_peers_last_backfill(hobject_t::get_max());
  } else {
    update_peers_last_backfill(obj);
  }
}

void BackfillState::ProgressTracker::update_peers_last_backfill(
