                 ...
                 const uint32_t flags)
{
  ...
  reads[hoid].emplace_back(
    boost::make_tuple(aligned_off, aligned_len, flags));
  seastar::promise<ceph::bufferlist> promise;
  auto ret = promise.get_future();
  objects_read_and_reconstruct(
    reads,
    fast_read,
    make_gen_lambda_context<std::map<hobject_t, std::pair<int, extent_map>>&&>(
...
ECBackend::handle_rep_read_op(ECSubRead& op)
{
  ...
  return seastar::do_with(ECSubReadReply{},
    ...
    return interruptor::do_for_each(op.to_read, [&op, &reply, FNAME, this] (auto read_item) {
      ...
      return interruptor::do_for_each(op_list, [&op, &reply, obj=obj, FNAME, this] (auto op_spec) {
