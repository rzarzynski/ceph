  ...
  ECListener& pg)
{
  ...
  logger().debug("{}:{}", __func__, __LINE__);
  return store->do_transaction(coll, std::move(txn)).then([this] {
    logger().debug("{}:{}", __func__, __LINE__);
  ...
  ECListener& eclistener)
{
  ...
  std::ignore = handle_sub_write(
    ...
    handle_rep_write_reply(std::move(reply));
