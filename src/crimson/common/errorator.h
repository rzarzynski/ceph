...
// TODO: consider specializing the templates instead of `plainify`-ing.
...
public:
  ...
  void handle() {
    ...
    if (type_info == ErrorT::error_t::get_exception_ptr_type_info()) {
      ...
      using ReturnType = std::invoke_result_t<ErrorVisitorT, ErrorT>;
...
private:
  ...
  public:
    ...
    template <template <class...> class ErroratedFuture,
              class = std::void_t<
                typename ErroratedFuture<
                  ::ceph::errorated_future_marker<ValuesT...>>::errorator_type>>
    ...
    auto safe_then(ValueFuncT&& valfunc, ErrorVisitorT&& errfunc) {
      ...
      using ValueFuncResult = std::invoke_result_t<ValueFuncT, ValuesT&&...>;
