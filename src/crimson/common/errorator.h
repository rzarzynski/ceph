...
static inline auto do_for_each(Container& c, AsyncAction action) {
  ...
  using ActionReturn = std::result_of_t<AsyncAction(ActionItem&)>;
...
static inline auto do_with(T&& rvalue, F&& f) {
  //using FuncReturn = std::result_of_t<F(T&)>;
...
public:
  ...
  void handle() {
    ...
    if (type_info == ErrorT::error_t::get_exception_ptr_type_info()) {
      ...
      using ReturnType = std::result_of_t<ErrorVisitorT(ErrorT)>;
      if constexpr (std::is_assignable_v<decltype(result), ReturnType>) {
        ...
        static_assert(_impl::always_false<ReturnType>::value,
...
private:
  ...
  class _future {};
  template <class... ValuesT>
  class _future<::ceph::errorated_future_marker<ValuesT...>> : private seastar::future<ValuesT...> {
    ...
    template <class... ValueFuncAllowedErrors>
    struct make_errorator<errorator<ValueFuncAllowedErrors...>> {
      using type = ::ceph::errorator<ValueFuncAllowedErrors...>;
    };
    template <class... Args>
  ...
  public:
    using errorator_type = ceph::errorator<AllowedErrors...>;
    ...
    template <template <class...> class ErroratedFuture,
              class = std::void_t<
                typename ErroratedFuture<::ceph::errorated_future_marker<ValuesT...>>::errorator_type>>
    operator ErroratedFuture<::ceph::errorated_future_marker<ValuesT...>> () && {
      using DestErrorator = \
        typename ErroratedFuture<::ceph::errorated_future_marker<ValuesT...>>::errorator_type;
      static_assert(DestErrorator::template contains_once_v<errorator_type>,
    ...
    auto safe_then(ValueFuncT&& valfunc, ErrorVisitorT&& errfunc) {
      ...
      using ReturnErrorator = make_errorator_t<
        ValueFuncErrorator,
        std::decay_t<std::result_of_t<ErrorVisitorT(AllowedErrors)>>...>;
      ...
      //     and back here or
      //   * specialize the `seastar::futirize<T>` to get proper kind of
      ...
      return this->then_wrapped(
        [ valfunc = std::forward<ValueFuncT>(valfunc),
          ...
          if (__builtin_expect(future.failed(), false)) {
            return _safe_then_handle_errors<Futurator>(
            ...
            // `::then()` already does.
            return Futurator::apply(std::forward<ValueFuncT>(valfunc),
                                    std::move(future).get());
          }
        });
    }
...
public:
  ...
  // to specializing `std::hash` for category-of-types-at-once which
  // was asked on SO: https://stackoverflow.com/a/38860413.
  template <class... ValuesT>
  using future = _future<::ceph::errorated_future_marker<ValuesT...>>;

  template <class... ValuesT, class ErrorT>
  static future<ValuesT...> make_exception_future(ErrorT&& e) noexcept {
  ...
  static decltype(auto) all_same_way(ErrorFunc&& error_func) {
    return [
      ...
      using DecayedT = std::decay_t<decltype(e)>;
      auto&& handler =
        DecayedT::error_t::handle(std::forward<ErrorFunc>(error_func));
...
private:
  ...
  template <template <class...> class ErroratedFutureT,
            class... ValuesT>
  class futurize<ErroratedFutureT<::ceph::errorated_future_marker<ValuesT...>>,
                 std::void_t<
                   typename ErroratedFutureT<::ceph::errorated_future_marker<ValuesT...>>::errorator_type>> {
  public:
    using type = ::ceph::errorator<AllowedErrors...>::future<ValuesT...>;
...
// in `future<...>::safe_then()`. See the comment there for details.
namespace seastar {

template <template <class...> class Container,
          class... Values>
struct futurize<Container<::ceph::errorated_future_marker<Values...>>> {
  using errorator_type = typename Container<
    ::ceph::errorated_future_marker<Values...>>::errorator_type;

  using type = typename errorator_type::template future<Values...>;
  using promise_type = promise<Values...>;
  ...
  static inline type convert(Values&&... values) { return make_ready_future<Values...>(std::move(values)...); }
