...
void bench_bufferlist_alloc(int size, int num, int per)
{
  ...
  for (int i=0; i<num; ++i) {
    bufferlist bl;
    for (int j=0; j<per; ++j)
      bl.push_back(buffer::hangable_ptr::create(buffer::create(size)));
  }
  utime_t end = ceph_clock_now();
  cout << num << " alloc of size " << size
       << " in " << (end - start) << std::endl;
...
TEST(BufferList, append_zero) {
  ...
  EXPECT_EQ((unsigned)1, bl.length());
  bl.append_zero(1);
  // TODO: we might want to have a bit more complex policy for 0-duduplication
  // than just "always do it".
  EXPECT_EQ((unsigned)2, bl.get_num_buffers());
  EXPECT_EQ((unsigned)2, bl.length());
  EXPECT_EQ('\0', bl[1]);
}
...
TEST(BufferList, ContiguousAppender) {
  ...
  // we expect a flush in ~contiguous_appender
  {
    ...
    denc(24l, ap);
    EXPECT_EQ(bl.get_num_buffers(), 3);
    EXPECT_EQ(bl.length(), sizeof(long) + 3);
  }
  EXPECT_EQ(bl.length(), 2 * sizeof(long) + 3);
}
