  ...
  struct test_state {
    struct Server final
      ...
      seastar::future<> init(const entity_name_t& name,
        ...
        return create_sharded<ceph::net::SocketMessenger>(name)
    ...
    struct Client final
      ...
      seastar::future<> ms_handle_connect(ceph::net::ConnectionRef conn) override {
        logger().info("Conn[{}] connected to {}", &*conn, conn->get_peer_addr());
      ...
      seastar::future<> ms_dispatch(ceph::net::ConnectionRef c,
        ...
        if (session->count == rounds) {
          logger().info("Conn[{}] finished with {} pingpongs", c.get(), session->count);
     ...
     private:
      seastar::future<> do_dispatch_pingpong(ceph::net::Connection* conn) {
        return seastar::do_with(unsigned(0), [this, conn](auto &count) {
            return seastar::do_until(
              [this, &count] { return count == rounds; },
              [this, conn, &count] {
                return seastar::repeat([this, conn, &count] {
  ...
  return seastar::when_all(
    ...
    .then([rounds, keepalive_ratio](result_tuple t) {
      ...
      return seastar::when_all_succeed(
          server1->init(entity_name_t::OSD(0), addr1),
          server2->init(entity_name_t::OSD(1), addr2),
          client1->init(entity_name_t::OSD(10)),
          client2->init(entity_name_t::OSD(11)))
      // dispatch pingpoing
